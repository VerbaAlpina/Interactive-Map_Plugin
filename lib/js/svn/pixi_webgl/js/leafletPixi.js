				...
				this.LpixiOverlay = L.pixiOverlay(function(utils) {
					...
					if (!firstDraw && prevZoom !== zoom) {
						...
						for(var i=0; i<that.pixipolygons.length;i++){
						    ...
						    var pixipolygon = that.pixipolygons[i];		
							that.drawPixiPolygonOutline(pixipolygon);
						}
	...
	if(that.polyhover){
		...
		pixi_polygon.on('mouseout',function(){
			...
			setTimeout(function() {
				...
				if(that.active_polygon == pixi_polygon && !that.performing_unhover){
						if(!that.alreadychecking_out){
							 checkOutside(that.current_latlng);	
					}
				}
...
this.drawPixiPolygonOutline = function(pixipolygon){
		...
		var line_width_fac = that.scale + (1/zoom)/25;
		var new_linewidth = pixipolygon['line_width'] / (line_width_fac);

		pixipolygon.updateLineStyle(new_linewidth, pixipolygon['stroke_color'], pixipolygon['stroke_alpha']);
