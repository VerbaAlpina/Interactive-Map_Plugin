...
function getThickLineArray(coords_array){
var res = [];
	...
	for(var i =0; i<coords_array.length;i+=2){
		...
		var n = calculatePerpendicular(x1,x2,y1,y2);
		var h = 2.0;
...
function calculatePerpendicular(x1,x2,y1,y2){
	...
	var n = {};
	n.x = -v.y / (Math.sqrt(Math.pow(v.x,2) + Math.pow(v.y,2))*0.00001);
	n.y = v.x / (Math.sqrt(Math.pow(v.x,2) + Math.pow(v.y,2))*0.00001);
